...
# Temporally disable the OCL, as there are some of AMD only packages existed inside install_NEO_OCL_driver.sh
...
RUN python3 -m pip install --no-cache-dir --upgrade pip; \
    ...
    pip install tensorflow-aarch64~=2.5.0 -f https://tf.kmtea.eu/whl/stable.html ; \
    python3 -m pip install --no-cache-dir 'openvino_dev[caffe,kaldi,onnx,pytorch]==2022.1.0' --find-links=tools/
...
FROM ubuntu:20.04 AS final_openvino
...
COPY --from=opencv ${INTEL_OPENVINO_DIR} ${INTEL_OPENVINO_DIR}
COPY --from=opencv /opt/libusb-1.0.22 /opt/libusb-1.0.22
...
RUN chmod +x install_dependencies/install_NCS_udev_rules.sh; \
    ...
    ./install_dependencies/install_NCS_udev_rules.sh; \
    echo " /lib/systemd/systemd-udevd --daemon " >> ~/.bashrc; \
    echo " /opt/intel/openvino/install_dependencies/install_NCS_udev_rules.sh" >> ~/.bashrc; \
...
WORKDIR ${INTEL_OPENVINO_DIR}
RUN ./demos/build_demos.sh; \
    cd demos/security_barrier_camera_demo/cpp; \
